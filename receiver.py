...
Features: Fullscreen Frameless Mode, Local Script / Executable Launcher with JSON History Memory,
          ...
          Dynamic Audio Playback, Native Win32 / Universal Input Injection (evdev/uinput/pynput),
...
class TouchDisplayCanvas(QWidget):
    """
    High-DPI Canvas with multi-touch and full multi-button mouse support (Left, Right, Middle).
    Suppresses synthetic touch mouse events on Windows 11 while reliably forwarding real clicks.
    """
    ...
    def mousePressEvent(self, event):
        ...
        # Ctrl+Right-Click or Alt+Right-Click opens receiver context menu while streaming
        ...
        if norm:
            ...
            self.control_server.send_event(
                {"type": "mouse_down", "x": norm[0], "y": norm[1], "button": btn}
            )
    ...
    def mouseReleaseEvent(self, event):
        ...
        if norm:
            ...
            self.control_server.send_event(
                {"type": "mouse_up", "x": norm[0], "y": norm[1], "button": btn}
            )
...
class ReceiverMainWindow(QMainWindow):
    def __init__(self):
        ...
        # Initialize local universal input injector for reverse viewer control
        w, h, l, t = self._get_active_monitor_geom()
        try:
            self.input_injector = UniversalInputInjector(w, h, mon_left=l, mon_top=t)
        except Exception as ex:
            logger.warning(f"Could not initialize local input injector on receiver: {ex}")
    ...
    def _get_active_monitor_geom(self) -> tuple[int, int, int, int]:
        try:
            with create_mss_instance() as sct:
                monitor = sct.monitors[1] if len(sct.monitors) > 1 else sct.monitors[0]
                return (
                    monitor.get("width", 1920),
            ...
            if screen:
                ...
                return (
                    max(1, int(round(geom.width() * dpr))),
                    max(1, int(round(geom.height() * dpr))),
    ...
    def on_control_command(self, cmd: dict):
        ...
        # Remote user input from Viewer window (mouse clicks, moves, scroll, keys)
        if cmd_type == "remote_input":
            event_data = cmd.get("event")
            if event_data and self.input_injector:
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self, "input_injector") and self.input_injector:
